...
def generate(r):
	...
	for i in r:
		...
		if n > 0 and n < 129:
			...
			melody.append(midi.NoteOnEvent(tick=t, data=[n-1, v]))
		if n == 129:
			v = 127 if i[n] != -1 else 0
			melody.append(midi.ControlChangeEvent(tick=t, data=[64, v]))
		if n == 130:
			tempo.append(midi.SetTempoEvent(tick=t, data=to_base_256(i[n])))
	print "okay"

	#Insert other events
	tempo.insert(0, midi.KeySignatureEvent())
	tempo.insert(0, midi.TimeSignatureEvent(data=[3, 3, 12, 8]))#Change in future
	...
	melody.insert(0, midi.TextMetaEvent(tick=0, text='bdca426d104a26ac9dcb070447587523', data=[98, 100, 99, 97, 52, 50, 54, 100, 49, 48, 52, 97, 50, 54, 97, 99, 57, 100, 99, 98, 48, 55, 48, 52, 52, 55, 53, 56, 55, 53, 50, 51]))#wtf?
	melody.insert(0, midi.ControlChangeEvent(data=[91, 127]))
	melody.insert(0, midi.ControlChangeEvent(data=[10, 64]))
	melody.insert(0, midi.ControlChangeEvent(data=[7, 100]))
	melody.insert(0, midi.ProgramChangeEvent())	
	melody = midi.Track(melody)
	melody.append(midi.EndOfTrackEvent())
	print "Yeah"
	melody.tick_relative = False
	melody.make_ticks_rel()
	mid = midi.Pattern(format=1, resolution=480, tracks=[tempo, melody])
	print "Fucking neato"
	return mid
